...
<body>
	<form action="/recipe/registerRecipe.do" id="registerRecipe"
		...
		<div class="register_recipe_wrapper">
				...
				<div class="wrapper_box pad_l_60">
					<div id="MainImageBox" class="cont_pic2">
						...
						<div style="position: absolute; left: -3000px">
							<input type="file" name="image" id="mainImage" file_gubun="main"
								accept="jpeg,png,gif"
								style="display:; width: 100px; height: 280px; font-size: 0px;"
								text="">
						</div>
					...
					<div class="wrapper_line">
						...
						<input type="text" name="recipeName" id="recipeName"
							...
							style="width: 620px; margin-top: 13px; display: inline-block;"
					...
					<div class="wrapper_line pad_b_25">
						...
						<textarea name="recipeDescription" id="recipeDescription"
							...
							style="height: 100px; width: 620px; resize: none;"><c:out
					...
					<div class="wrapper_line">
						...
						<select class="form-select" id="foodType" name="foodType"
							...
							<option value="한식"
								<c:if test="${foodType == '한식'}}">selected</c:if>>한식</option>
							<option value="중국"
								<c:if test="${foodType == '중국'}">selected</c:if>>중국</option>
							<option value="일본"
								<c:if test="${foodType == '일본'}">selected</c:if>>일본</option>
							<option value="서양"
								<c:if test="${foodType == '서양'}">selected</c:if>>서양</option>
							<option value="이탈리아"
								<c:if test="${foodType == '이탈리아'}">selected</c:if>>이탈리아</option>
							<option value="동남아시아"
								<c:if test="${foodType == '동남아시아'}">selected</c:if>>동남아시아</option>
							<option value="퓨전"
								<c:if test="${foodType == '퓨전'}">selected</c:if>>퓨전</option>
				...
				<div class="wrapper_box pad_l_60">
					<div class="mag_b_25 ui-sortable" id="divIngredientGroupArea">
						<div class="wrapper_line" style="height: 0px; display: flex">
							<p class="wrapper_title_ingredient"
								style="color: black; margin-top: 5px;">재료</p>
						</div>
						<div class="wrapper_ingredient"
							style="margin-left: 145px; display: inline-flex;">
							<ul id="ingredientAra_1" class="ingredientGroup">
								<select class="form-select" id="ingreType" name="ingreType"
									control-id="ControlID-11" style="justify-content: flex-end;">
									<option value="">재료별</option>
									<option value="주재료"
										<c:if test="${ingreType == '주재료'}">selected</c:if>>주재료</option>
									<option value="부재료"
										<c:if test="${ingreType == '부재료'}">selected</c:if>>부재료</option>
									<option value="양념"
										<c:if test="${ingreType == '양념'}">selected</c:if>>양념</option>
								</select>
								<li id="liIngredient_1_1"><input type="text"
									...
									style="width: 275px; border-radius: 0.375rem"
									...
									style="width: 180px; border-radius: 0.375rem"
									placeholder="예) 300g" control-id="ControlID-20">
									<button type="button"
										class="btn btn-warning deleteIngredientBtn">X</button></li>
							</ul>
						</div>
					</div>
					<div class="btn_add" style="padding: 0 0 20px 400px; witdth: 800px">
						<button type="button" onclick="addIngredient(1)"
							...
							style="width: 100px; height: 50px; letter-spacing: .3rem; font-size: 20px;">
				...
				<div class="wrapper_box2 pad_l_60">
					...
					<div id="stepArea" class="stepArea">
						<div id="stepItem_1" class="step" style="margin-bottom: 50px">
							<p id="stepNum_1" name="stepNo" class="stepNo"
								data-original-title="" title=""
								style="width: 100px; display: inline-block; font-size: 20px; font-weight: bold; vertical-align: top; color: #black; margin-left: 40px;">Step
								1</p>
							<div id="wrapper_stepDescription_1" style="display: inline-block">
								<textarea name="stepDescription" id="stepDescription_1"
									class="form-control stepDescription"
									placeholder="예) 김치를 적당한 크기로 썰어 팬에 볶아주세요."
									style="height: 200px; width: 410px; resize: none;"
							...
							<div id="stepUpload_1" style="display: inline-block">
								<input type="hidden" name="step_no" id="step_no_1" value=""
									...
									name="step_image" id="step_image_1" value=""> <input
									type="hidden" name="new_step_image" id="new_step_image_1"
									value=""> <input type="hidden" name="del_step_image"
									id="del_step_image_1" value="">
								<div style="position: absolute; left: -3000px">
									<input type="file" name="stepImage" id="q_step_file_1"
										file_gubun="step" accept="jpeg,png,gif"
										style="display: none; width: 0px; height: 0px; font-size: 0px;"
										text="" control-id="ControlID-31">
								</div>
								<div id="stepImageBox_1" is_over="0" onclick="browseStepFile(1)"
									style="cursor: pointer">
									<img id="stepImageHolder_1"
										src="../resources/assets/uploadimage.png"
					...
					<div id="stepTemplate" style="display: none;">
						<div id="stepItem_STEP" class="step" style="margin-bottom: 50px">
							...
							<div id="tipItem_STEP" class="tip">
								...
								<button type="button" class="btn btn-warning deleteStepBtn"
									onclick="deleteStep(STEP)" id="deleteStepBtn_STEP">X</button>
							</div>
						</div>
					</div>
					<!--step template finish-->
					<div class="btn_add mag_b_25"
						style="padding: 0 0 20px 380px; width: 820px;">
						<button type="button" onclick="addStep()" class="btn btn-primary"
							control-id="ControlID-83"
							style="background-color: #e95420; width: 150px; height: 50px; font-size: 20px; letter-spacing: 6px;">
			...
			<div class="regi_btm">
				<button type="submit" onclick="save()" class="btn-lg btn-primary"
					id="save" control-id="ControlID-44"
					style="width: 150px; letter-spacing: 8px; font-size: 25px;">저장</button>
				<button type="button" onclick="location.href='/recipe/get'"
					class="btn-lg btn-warning" id="cancel" control-id="ControlID-49"
					style="width: 150px; font-size: 25px; letter-spacing: 8px;">취소</button>
			</div>
			<!--save and cancel-->
		</div>
		<!--container finish-->
